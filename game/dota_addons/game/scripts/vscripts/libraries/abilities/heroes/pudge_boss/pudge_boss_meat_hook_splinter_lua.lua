pudge_boss_meat_hook_splinter_lua = class({})
LinkLuaModifier( "modifier_pudge_boss_meat_hook_splinter_followthrough_lua", "libraries/modifiers/pudge_boss/modifier_pudge_boss_meat_hook_splinter_followthrough_lua.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pudge_boss_meat_hook_splinter_lua", "libraries/modifiers/pudge_boss/modifier_pudge_boss_meat_hook_splinter_lua.lua", LUA_MODIFIER_MOTION_HORIZONTAL )
...
function pudge_boss_meat_hook_splinter_lua:OnSpellStart()
	...
	for k, v in pairs (tHeroes) do
		if not ( v:HasModifier( "modifier_pudge_boss_meat_hook_lua" ) or v:HasModifier( "modifier_pudge_boss_meat_hook_splinter_lua" ) )then
			--print( self:GetCaster():GetName().." "..self:GetCaster():GetEntityIndex() )
			--print( v:GetName().." "..v:GetEntityIndex() )
	...
	if self.hTarget then 
		...
		self.hAttacker = self:GetCaster():FindModifierByName("modifier_pudge_boss_meat_hook_lua"):GetCaster()
	end
end

--------------------------------------------------------------------------------

function pudge_boss_meat_hook_splinter_lua:OnProjectileHit( hTarget, vLocation )
	if hTarget == self:GetCaster() then
		return false
	end

	if self.bRetracting == false then
		if hTarget ~= nil and not hTarget:IsRealHero() then
		...
		if hTarget ~= nil then
			...
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_pudge_boss_meat_hook_splinter_lua", nil )
		...
		if self.hVictim ~= nil then
			...
			self.hVictim:InterruptMotionControllers( true )
			self.hVictim:RemoveModifierByName( "modifier_pudge_boss_meat_hook_splinter_lua" )
		...
		self:GetCaster():RemoveAbility("pudge_boss_meat_hook_splinter_lua")
	end
...
function pudge_boss_meat_hook_splinter_lua:OnProjectileThink( vLocation )
	self.vProjectileLocation = vLocation
end

--------------------------------------------------------------------------------

function pudge_boss_meat_hook_splinter_lua:OnOwnerDied()
	self:GetCaster():RemoveGesture( ACT_DOTA_OVERRIDE_ABILITY_1 );
