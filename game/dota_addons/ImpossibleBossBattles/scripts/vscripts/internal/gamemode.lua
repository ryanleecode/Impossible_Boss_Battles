...
function GameMode:_InitGameMode()
    ...
    --Filters
    GameRules:GetGameModeEntity():SetTrackingProjectileFilter( Dynamic_Wrap( GameMode, "FilterProjectile" ), self )
...
function GameMode:FilterProjectile( filterTable )
    ...
    local isAttack = tobool(filterTable["is_attack"])
    local projectileSpeed = filterTable["move_speed"]

    -- Executes if it is an attack
    if isAttack then
        if attacker:HasArtilleryAttack() then
            AttackGroundPos( attacker, victim:GetAbsOrigin() )
            return false
        end
    -- Executes if it is a spell
